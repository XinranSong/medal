...
from utils2 import load_mimic_mortality, load_mimic_diagnosis, load_model, predict, evaluate, train_loop
...
if __name__ == '__main__':
    ...
    if not TEST:
        ...
        # Train network
        net, logs, valid_outputs, valid_labels, valid_outputList, valid_labelList = train_loop(
        ...
        if TASK == 'mimic-mortality':
            ...
            test_loss, test_metric, test_outputs, test_labels, test_outputList, test_labelList = evaluate(net, test_loader, test_data, criterion, verbose=True, task=TASK)
    ...
    # Save Outputs valid_outputs, valid_labels, valid_outputList, valid_labelList
    outputs_df = pd.DataFrame(valid_outputs)
    outputs_df.to_csv(os.path.join(save_dir, 'outputs.csv'))

    labels_df = pd.DataFrame(valid_labels)
    labels_df.to_csv(os.path.join(save_dir, 'labels.csv'))

    outputList_df = pd.DataFrame(valid_outputList)
    outputList_df.to_csv(os.path.join(save_dir, 'outputList.csv'))

    labelList_df = pd.DataFrame(valid_labelList)
    labelList_df.to_csv(os.path.join(save_dir, 'labelList.csv'))
